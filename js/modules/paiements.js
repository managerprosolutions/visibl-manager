...
document.addEventListener("DOMContentLoaded",()=>{if(typeof requireAuth==="function"&&!requireAuth())return;initialiserPaiements();chargerPaiements();});
...
function appliquerFiltresPaiements(){const q=nrm(val("payments-search-input")),o=nrm(val("payment-origin-filter")),m=nrm(val("payment-method-filter")),n=nrm(val("payment-nature-filter")),s=nrm(val("payment-status-filter"));paiementsFiltres=paiementsCharges.filter(p=>{const t=nrm([p.idPaiement,p.idVente,p.idLivraison,p.origine,p.referencePaiement,p.naturePaiement,p.modePaiement,p.statut,p.idUtilisateur,p.commentaire].join(" "));return(!q||t.includes(q))&&(!o||nrm(p.origine)===o)&&(!m||normMode(p.modePaiement)===m)&&(!n||nrm(p.naturePaiement)===n)&&(!s||normStatut(p.statut)===s);});txt("filtered-payment-count",paiementsFiltres.length);pagePaiementsActuelle=1;afficherTableauPaiements();}
